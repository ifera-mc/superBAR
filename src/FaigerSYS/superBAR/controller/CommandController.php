...
abstract class CommandController{
	 ...
	 * @return bool
	 */
	public static function executeCommand(superBAR $plugin, CommandSender $sender, array $args){
		$NL = ($sender instanceof ConsoleCommandSender) ? PHP_EOL : "\n";
		$command = array_shift($args);
		if($command === null){
			$sender->sendMessage($plugin::PREFIX . 'Version ' . $plugin->getDescription()->getVersion() . $NL . CLR::GRAY . 'Commands list: ' . CLR::DARK_GREEN . '/sb help');
		}elseif($command === 'help'){
			if(!$plugin->hasPermission($sender, 'help')){
				return $sender->sendMessage($plugin::PREFIX . $plugin::NO_PERM);
			}
			$sender->sendMessage($plugin::PREFIX . 'Commands:' . $NL . CLR::DARK_GREEN . '/sb enable' . CLR::GREEN . ' or ' . CLR::DARK_GREEN . '/sb on' . CLR::BLUE . ' - ' . CLR::DARK_AQUA . 'enable HUD for you' . $NL . CLR::DARK_GREEN . '/sb disable' . CLR::GREEN . ' or ' . CLR::DARK_GREEN . '/sb off' . CLR::BLUE . ' - ' . CLR::DARK_AQUA . 'disable HUD for you' . $NL . CLR::DARK_GREEN . '/sb change' . CLR::GREEN . ' or ' . CLR::DARK_GREEN . '/sb set' . CLR::BLUE . ' - ' . CLR::DARK_AQUA . 'change HUD settings' . $NL . CLR::DARK_GREEN . '/sb reload' . CLR::BLUE . ' - ' . CLR::DARK_AQUA . 'reload the superBAR settings');
